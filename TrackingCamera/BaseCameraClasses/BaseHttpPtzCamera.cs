...
namespace TrackingCamera.BaseCameraClasses
{
	...
	/// </summary>
	public abstract class BaseHttpPtzCamera : BasePtzCamera, IHttpCamera, IPtzCamera
	{
		...
		#region IHttpCamera members

		public object PtzController { get; set; }
		...
		/// <returns></returns>
		static async Task AsyncHelper(BaseHttpPtzCamera camera)
		{
		...
		public BaseHttpPtzCamera(string cameraIpAddress, string userName, string password, string cameraName)
		...
		/// <param name="Password">the password to log in with</param>
		/// <param name="CameraName">the user friendly name of the camera</param>
		/// <param name="httpPort">the port number of the Http service</param>
		public BaseHttpPtzCamera(string cameraIpAddress, string userName, string password,
			string cameraName, int httpPort) 
			: this(cameraIpAddress, userName, password, cameraName)
		{
			this.HttpPort = httpPort;
			this.HostAddress = String.Format("{0}:{1}", this.CameraIpAddress, this.HttpPort);

			AsyncHelper(this).Wait();

			Globals.Log.Debug("HTTP Port: " + this.HttpPort.ToString());
		}
		#endregion
